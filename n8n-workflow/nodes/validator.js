...
function validate(transformResult) {
  ...
  // ── Also carry over any missing_fields flagged by Claude during extraction
  const missingFromExtraction = transformResult.purchase_order?.source_file
    ? []
    : [];
...
const items_input = $input.all();
const results = [];

for (const item of items_input) {
  const validated = validate(item.json);
